...
// reorder, and restructure pages/chapters. All changes stay on this machine.
...
export async function executeBookTool(
  ...
  switch (name) {
    ...
    case "insert_page": {
      ...
      // Allow insert at end (before_page === length + 1).
    ...
    case "delete_page": {
      ...
      if (m.pages.length === 1)
        return {
          summary: "Can't delete the only page — write over it or replace the structure.",
        };
